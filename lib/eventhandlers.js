...
var EventHandlers = {
	keydownEvent: function (e) {
		...
		inputmask.ignorable = opts.ignorables.includes(k) && (e.key == null || e.key.length !== 1);
	},
	...
	pasteEvent: function (e) {
		...
		writeBuffer(input, getBuffer.call(inputmask), seekNext.call(inputmask, getLastValidPosition.call(inputmask)), e, inputmask.undoValue !== getBuffer.call(inputmask).join(""));
		return e.preventDefault();
	},
	...
	mouseenterEvent: function () {
		...
		if ((input.inputmask.shadowRoot || document).activeElement !== input) {
			var value = (inputmask.isRTL ? getBufferTemplate.call(inputmask).slice().reverse() : getBufferTemplate.call(inputmask)).join("");
			if (inputmask.originalPlaceholder == undefined && input.placeholder !== inputmask.originalPlaceholder && input.placeholder !== value) {
